...
> A client receives the configuration listed in the table through the initial exchange.
>
> - **Check:** verify that the acknowledged address does not conflict with another address.
> - **Result:** when no conflict is found, use the listed address and network settings for the lease period.
