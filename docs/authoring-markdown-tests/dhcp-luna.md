...
sequenceDiagram
    ...
    participant R as Relay or local network
    participant S as DHCP server
    C->>R: Broadcast DHCPDISCOVER
    R->>S: Forward discover
    S-->>R: DHCPOFFER
    R-->>C: Offer, broadcast or unicast
    C->>R: Broadcast DHCPREQUEST
    R->>S: Forward request
    S-->>R: DHCPACK
    R-->>C: ACK, broadcast or unicast
